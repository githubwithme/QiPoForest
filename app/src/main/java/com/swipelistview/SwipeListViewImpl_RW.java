...
import com.bhq.ui.TaskContent_;
...
public class SwipeListViewImpl_RW implements OnLayoutAnimatListener
{
	...
	{
		dt_manager = AppContext.getUserInfo(context);
		this.from = from;
	...
	private View newConvertView(int position)
	{
		...
		holder.message_contents.setOnClickListener(new OnClickListener()
		{
			...
			{
				Intent intent = new Intent(context, TaskContent_.class);
		...
		if (dt_manager.getid().equals(list_RW_RW.get(mposition).getZRR()))
		{
	...
	private void generateUi(int position, RW_RW RW_RW, ViewHolder viewHolder)
	{
		...
		status = utils.getDayDifference(utils.getToday(), utils.DateString2Date(list_RW_RW.get(position).getWRJZSJ()));
