...
public class SqliteDb
{
	...
	public static void existSystemoffline(Context context, dt_manager_offline dt_manager_offline)// 这个方式可以
	{
		...
		try
		{
			db.update(dt_manager_offline, "AutoLogin");
		} catch (DbException e)
		{
			e.printStackTrace();
		}
	...
	public static List<RW_RW> getRenWuByRYID(Context context, String RYID)
	{
		...
		try
		{
			String sql = "SELECT  * FROM RW_RW LEFT OUTER JOIN RW_CYR ON RW_RW.RWID = RW_CYR.RWID WHERE RYID = " + RYID + " AND RW_CYR.SFSC='false' and HYSFQX='false' or ZRR = " + RYID + " ORDER BY RWMC ASC";
