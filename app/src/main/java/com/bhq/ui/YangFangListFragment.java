...
import com.bhq.R;
import com.bhq.adapter.ListViewYangFangAdapter;
 ...
 */
@EFragment
public class YangFangListFragment extends Fragment
{
	private AppContext appContext;// 全局Context
	private List<QiaoMuTree> listData = new ArrayList<QiaoMuTree>();
	private ListViewYangFangAdapter listAdapter;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState)
	{
		...
		IntentFilter refresh_Filter = new IntentFilter(AppContext.ACTION_REFRESH_FACILITY);
	...
	private void TestListData(final int actiontype, final int objtype, final PullToRefreshListView lv, final BaseAdapter adapter, final TextView more, final ProgressBar progressBar, final int PAGESIZE, int PAGEINDEX)
	{
		String result = utils.parseJsonFile(getActivity(), "qmtree.json");
		List<QiaoMuTree> listNewData = null;
		listNewData = JSON.parseArray(result, QiaoMuTree.class);
		if (listNewData == null)
		{
			listNewData = new ArrayList<QiaoMuTree>();
		}
		// 数据处理
		...
		case UIHelper.LISTVIEW_ACTION_CHANGE_CATALOG:// 页面切换
			...
			case UIHelper.LISTVIEW_DATATYPE_NEWS:
				...
				if (actiontype == UIHelper.LISTVIEW_ACTION_REFRESH)
				{
					...
					{
						for (QiaoMuTree qiaoMuTree1 : listNewData)
						{
							boolean b = false;
							for (QiaoMuTree qiaoMuTree2 : listData)
							{
								if (qiaoMuTree1.getQMID().equals(qiaoMuTree2.getQMID()))
								{
									b = true;
									break;
								}
			...
			case UIHelper.LISTVIEW_DATATYPE_NEWS:
				...
				if (listNewData.size() > 0)
				{
					for (QiaoMuTree qiaoMuTree1 : listNewData)
					{
						...
						for (QiaoMuTree qiaoMuTree2 : listData)
						{
							if (qiaoMuTree1.getQMID() == qiaoMuTree2.getQMID())
							{
								b = true;
								break;
							}
	...
	private void getListData(final int actiontype, final int objtype, final PullToRefreshListView lv, final BaseAdapter adapter, final TextView more, final ProgressBar progressBar, final int PAGESIZE, int PAGEINDEX)
	{
		...
		String params = ConnectionHelper.setParams("APP.GetPlantList", "0", hashMap);
		new HttpUtils().send(HttpRequest.HttpMethod.POST, AppConfig.dataBaseUrl, ConnectionHelper.getParas(params), new RequestCallBack<String>()
		{
			...
			public void onSuccess(ResponseInfo<String> responseInfo)
			{
				...
				{
					listNewData = JSON.parseArray(ResultDeal.getAllRow(result), QiaoMuTree.class);
					if (listNewData == null)
					{
						listNewData = new ArrayList<QiaoMuTree>();
					}
	...
	{
		listAdapter = new ListViewYangFangAdapter(getActivity(), listData, R.layout.yangfang_listitem);
		...
		frame_listview_news.setOnItemClickListener(new AdapterView.OnItemClickListener()
		{
			public void onItemClick(AdapterView<?> parent, View view, int position, long id)
			{
				...
				// if (animal == null)
				// return;
				QiaoMuTree qiaoMuTree = listData.get(position - 1);
				if (qiaoMuTree == null)
					return;
				Intent intent = new Intent(getActivity(), ShowQiaoMu_.class);
				intent.putExtra("qiaoMuTree", qiaoMuTree);// 因为list中添加了头部,因此要去掉一个
		...
		frame_listview_news.setOnScrollListener(new AbsListView.OnScrollListener()
		{
			public void onScrollStateChanged(AbsListView view, int scrollState)
			{
				...
				if (scrollEnd && lvDataState == UIHelper.LISTVIEW_DATA_MORE)
				{
					...
					TestListData(UIHelper.LISTVIEW_ACTION_SCROLL, UIHelper.LISTVIEW_DATATYPE_NEWS, frame_listview_news, listAdapter, list_foot_more, list_foot_progress, AppContext.PAGE_SIZE, pageIndex);
		...
		frame_listview_news.setOnRefreshListener(new PullToRefreshListView.OnRefreshListener()
		{
			public void onRefresh()
			{
				...
				TestListData(UIHelper.LISTVIEW_ACTION_REFRESH, UIHelper.LISTVIEW_DATATYPE_NEWS, frame_listview_news, listAdapter, list_foot_more, list_foot_progress, AppContext.PAGE_SIZE, 0);
			}
		});
		// 加载资讯数据
		if (listData.isEmpty())
		{
			TestListData(UIHelper.LISTVIEW_ACTION_INIT, UIHelper.LISTVIEW_DATATYPE_NEWS, frame_listview_news, listAdapter, list_foot_more, list_foot_progress, AppContext.PAGE_SIZE, 0);
		}
	}
}
