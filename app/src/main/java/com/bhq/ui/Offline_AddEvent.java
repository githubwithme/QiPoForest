...
public class Offline_AddEvent extends Activity
{
	@ViewById
	...
	void tv_type()
	{
		...
		{
			Toast.makeText(Offline_AddEvent.this, "数据获取异常，请重试！", Toast.LENGTH_SHORT).show();
		}
	}
	...
	{
		Intent intent = new Intent(Offline_AddEvent.this, HomeFragmentActivity.class);
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		dt_manager_offline = (com.bhq.bean.dt_manager_offline) SqliteDb.getCurrentUser(Offline_AddEvent.this, dt_manager_offline.class);
	...
	BroadcastReceiver imageBroadcastReceiver = new BroadcastReceiver()// 植物（0为整体照，1为花照，2为果照，3为叶照）；动物（0为整体照，1为脚印照，2为粪便照）
	{
		...
		public void onReceive(Context context, Intent intent)
		{
			...
			for (int i = 0; i < list.size(); i++)
			{
				...
				ImageView imageView = new ImageView(Offline_AddEvent.this);
				...
				BitmapHelper.setImageView(Offline_AddEvent.this, imageView, FJBDLJ);
				...
				imageView.setOnClickListener(new OnClickListener()
				{
					...
					public void onClick(View v)
					{
						...
						myDialog = new MyDialog(Offline_AddEvent.this, R.style.MyDialog, dialog_layout, "图片", "查看该图片?", "查看", "删除", new CustomDialogListener()
						{
							@Override
							public void OnClick(View v)
							{
			...
			for (int i = 0; i < list.size(); i++)
			{
				...
				imageView.setOnClickListener(new OnClickListener()
				{
					...
					public void onClick(View v)
					{
						...
						View dialog_layout = (LinearLayout) getLayoutInflater().inflate(R.layout.customdialog_callback, null);
						myDialog = new MyDialog(Offline_AddEvent.this, R.style.MyDialog, dialog_layout, "图片", "查看该视频?", "查看", "删除", new CustomDialogListener()
						{
							@Override
							public void OnClick(View v)
							{
	...
	private void uploadData()
	{
		...
		bhq_XHSJ.setSFSC("0");
		boolean issuccess = SqliteDb.save(Offline_AddEvent.this, bhq_XHSJ);
		if (issuccess)
		{
			if (list_allfj.size() > 0)
			{
				boolean success = SqliteDb.saveAll(Offline_AddEvent.this, list_allfj);
				if (success)
				{
					Toast.makeText(Offline_AddEvent.this, "保存成功！", Toast.LENGTH_SHORT).show();
					finish();
				} else
				{
					pb_upload.setVisibility(View.GONE);
					btn_upload.setVisibility(View.VISIBLE);
					Toast.makeText(Offline_AddEvent.this, "数据保存失败", Toast.LENGTH_SHORT).show();
				}
			} else
			{
				Toast.makeText(Offline_AddEvent.this, "保存成功！", Toast.LENGTH_SHORT).show();
		...
		} else
		{
			pb_upload.setVisibility(View.GONE);
			btn_upload.setVisibility(View.VISIBLE);
			Toast.makeText(Offline_AddEvent.this, "上传失败", Toast.LENGTH_SHORT).show();
		}
	...
	{
		list_dic = SqliteDb.getDics(Offline_AddEvent.this, Dictionary.class);
		if (list_dic == null)
		{
