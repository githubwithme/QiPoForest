...
import com.bhq.R;
import com.bhq.adapter.ListViewknowledgeAdapter;
...
public class KnowledgeFragment extends Fragment
{
	...
	private ListViewknowledgeAdapter listAdapter;
	...
	private void getListData(final int actiontype, final int objtype, final PullToRefreshListView lv, final BaseAdapter adapter, final TextView more, final ProgressBar progressBar, final int PAGESIZE, int PAGEINDEX)
	{
		...
		hashMap.put("ZSDL", ZYDL);
		String params = ConnectionHelper.setParams("APP.GetBHQ_ZSKList", "0", hashMap);
		new HttpUtils().send(HttpRequest.HttpMethod.POST, AppConfig.dataBaseUrl, ConnectionHelper.getParas(params), new RequestCallBack<String>()
		{
			...
			public void onSuccess(ResponseInfo<String> responseInfo)
			{
				...
				List<BHQ_ZSK> listNewData = null;
				Result result = JSON.parseObject(responseInfo.result, Result.class);
				if (result.getResultCode() == 200)// 连接数据库成功
				{
					listNewData = JSON.parseArray(ResultDeal.getAllRow(result), BHQ_ZSK.class);
					if (listNewData == null)
					{
	...
	{
		listAdapter = new ListViewknowledgeAdapter(getActivity(), listData);
		...
		frame_listview_news.setOnItemClickListener(new AdapterView.OnItemClickListener()
		{
			public void onItemClick(AdapterView<?> parent, View view, int position, long id)
			{
				...
				if (BHQ_ZSK == null)
					return;
				Intent intent = new Intent(getActivity(), ShowKnowledge_.class);
