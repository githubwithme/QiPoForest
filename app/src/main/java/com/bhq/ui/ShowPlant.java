...
public class ShowPlant extends Activity
{
	...
	class FoundationFragment extends Fragment
	{
		...
		public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState)
		{
			...
			tv_description.setText(plant.getSzzk());
			tv_bz.setText(plant.getBz());
			return rootView;
		}
...
//				ShowPlant.this.getFragmentManager().beginTransaction().replace(R.id.img_container, pictureScrollFragment).commit();
			...
			{
				Toast.makeText(ShowPlant.this, "获取失败", Toast.LENGTH_SHORT).show();
			}
		});
	}
	...
	private void getVideo(String glid)
	{
		...
		new HttpUtils().send(HttpRequest.HttpMethod.POST, AppConfig.dataBaseUrl, ConnectionHelper.getParas(params), new RequestCallBack<String>()
		{
			...
			public void onSuccess(ResponseInfo<String> responseInfo)
			{
				...
				for (int i = 0; i < list_fj.size(); i++)
				{
					ProgressBar progressBar = new ProgressBar(ShowPlant.this, null, android.R.attr.progressBarStyleHorizontal);
		...
		http.download(path, target, true, true, new RequestCallBack<File>()
		{
			...
			public void onFailure(HttpException error, String msg)
			{
				if (msg.equals("maybe the file has downloaded completely"))
				{
					...
					ImageView imageView = new ImageView(ShowPlant.this);
					LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(400, LayoutParams.MATCH_PARENT, 0); // ,
																														// 1是可选写的
					lp.setMargins(25, 4, 0, 4);
					imageView.setLayoutParams(lp);// 显示图片的大小
				...
				{
					Toast.makeText(ShowPlant.this, "下载失败！找不到文件!", Toast.LENGTH_SHORT).show();
				}
			}
			...
			public void onSuccess(ResponseInfo<File> responseInfo)
			{
				...
				ll_video.removeView(progressBar);
				Toast.makeText(ShowPlant.this, "下载成功！", Toast.LENGTH_SHORT).show();
				ImageView imageView = new ImageView(ShowPlant.this);
				LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(400, LayoutParams.MATCH_PARENT, 0); // ,
																													// 1是可选写的
				lp.setMargins(25, 4, 0, 4);
