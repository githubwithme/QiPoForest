...
import com.bhq.R;
import com.bhq.adapter.ListViewAnimalAdapter;
...
public class AnimalListFragment extends Fragment
{
	...
	private List<Animal> listData = new ArrayList<Animal>();
	private ListViewAnimalAdapter listAdapter;
	...
	private void getTestListData(final int actiontype, final int objtype, final PullToRefreshListView lv, final BaseAdapter adapter, final TextView more, final ProgressBar progressBar, final int PAGESIZE, int PAGEINDEX)
	{
		String result = utils.parseJsonFile(getActivity(), "data/animal.json");
		List<Animal> listNewData = null;
		listNewData = JSON.parseArray(result, Animal.class);
		if (listNewData == null)
		{
			listNewData = new ArrayList<Animal>();
		}
		// 数据处理
		...
		case UIHelper.LISTVIEW_ACTION_CHANGE_CATALOG:// 页面切换
			...
			case UIHelper.LISTVIEW_DATATYPE_NEWS:
				...
				if (actiontype == UIHelper.LISTVIEW_ACTION_REFRESH)
				{
					...
					{
						for (Animal animal1 : listNewData)
						{
							boolean b = false;
							for (Animal animal2 : listData)
							{
								if (animal1.getDwid().equals(animal2.getDwid()))
								{
									b = true;
									break;
								}
			...
			case UIHelper.LISTVIEW_DATATYPE_NEWS:
				...
				if (listNewData.size() > 0)
				{
					for (Animal animal1 : listNewData)
					{
						...
						for (Animal animal2 : listData)
						{
							if (animal1.getDwid() == animal2.getDwid())
							{
								b = true;
								break;
							}
	...
	private void getListData(final int actiontype, final int objtype, final PullToRefreshListView lv, final BaseAdapter adapter, final TextView more, final ProgressBar progressBar, final int PAGESIZE, int PAGEINDEX)
	{
		...
		new HttpUtils().send(HttpRequest.HttpMethod.POST, AppConfig.dataBaseUrl, ConnectionHelper.getParas(params), new RequestCallBack<String>()
		{
			...
			public void onSuccess(ResponseInfo<String> responseInfo)
			{
				...
				{
					listNewData = JSON.parseArray(ResultDeal.getAllRow(result), Animal.class);
					if (listNewData == null)
					{
						listNewData = new ArrayList<Animal>();
					}
	...
	{
		listAdapter = new ListViewAnimalAdapter(getActivity(), listData, R.layout.animal_listitem);
		...
		frame_listview_news.setOnItemClickListener(new AdapterView.OnItemClickListener()
		{
			public void onItemClick(AdapterView<?> parent, View view, int position, long id)
			{
				...
				// if (animal == null)
				// return;
				Animal animal = listData.get(position - 1);
				if (animal == null)
					return;
				Intent intent = new Intent(getActivity(), ShowAnimal_.class);
				intent.putExtra("animal", animal);// 因为list中添加了头部,因此要去掉一个
		...
		frame_listview_news.setOnScrollListener(new AbsListView.OnScrollListener()
		{
			public void onScrollStateChanged(AbsListView view, int scrollState)
			{
				...
				if (scrollEnd && lvDataState == UIHelper.LISTVIEW_DATA_MORE)
				{
					...
					getTestListData(UIHelper.LISTVIEW_ACTION_SCROLL, UIHelper.LISTVIEW_DATATYPE_NEWS, frame_listview_news, listAdapter, list_foot_more, list_foot_progress, AppContext.PAGE_SIZE, pageIndex);
		...
		frame_listview_news.setOnRefreshListener(new PullToRefreshListView.OnRefreshListener()
		{
			public void onRefresh()
			{
				...
				getTestListData(UIHelper.LISTVIEW_ACTION_REFRESH, UIHelper.LISTVIEW_DATATYPE_NEWS, frame_listview_news, listAdapter, list_foot_more, list_foot_progress, AppContext.PAGE_SIZE, 0);
			}
		});
		// 加载资讯数据
		if (listData.isEmpty())
		{
			getTestListData(UIHelper.LISTVIEW_ACTION_INIT, UIHelper.LISTVIEW_DATATYPE_NEWS, frame_listview_news, listAdapter, list_foot_more, list_foot_progress, AppContext.PAGE_SIZE, 0);
		}
	}
}
