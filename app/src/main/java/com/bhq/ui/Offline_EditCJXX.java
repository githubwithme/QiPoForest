...
public class Offline_EditCJXX extends Activity
{
	...
	void tv_bwd()
	{
		...
		{
			Toast.makeText(Offline_EditCJXX.this, "数据获取异常，请重试！", Toast.LENGTH_SHORT).show();
		}
	...
	{
		witchphoto = "ZW_HD";
		Intent intent = new Intent(Offline_EditCJXX.this, HomeFragmentActivity.class);
	...
	{
		ImageView imageView = new ImageView(Offline_EditCJXX.this);
		...
		BitmapHelper.loadImage(Offline_EditCJXX.this, imageView, fj_SCFJ.getFJBDLJ());
		...
		imageView.setOnClickListener(new OnClickListener()
		{
			...
			public void onClick(View v)
			{
				...
				myDialog = new MyDialog(Offline_EditCJXX.this, R.style.MyDialog, dialog_layout, "图片", "查看该图片?", "查看", "删除", new CustomDialogListener()
				{
					@Override
					public void OnClick(View v)
					{
						...
						{
						case R.id.btn_sure:
							Intent intent = new Intent(Offline_EditCJXX.this, Offline_ShowPhoto_.class);
							intent.putExtra("url", list_pic.get(index_zp).getFJBDLJ());
							startActivity(intent);
							break;
						case R.id.btn_cancle:
							deletePhotos(list_pic.get(index_zp), list_pic, ll_pic, index_zp);
							myDialog.dismiss();
							break;
						}
					}
	...
	private void deletePhotos(FJ_SCFJ fj_SCFJ, final List<FJ_SCFJ> list_pic, final LinearLayout ll_pic, final int index_zp)
	{
		...
		fj_SCFJ.setSFSC("1");
		boolean issuccess = SqliteDb.deletePhotos(Offline_EditCJXX.this, fj_SCFJ);
		if (issuccess)
		{
			ll_pic.removeViewAt(index_zp);
			list_pic.remove(index_zp);
			Toast.makeText(Offline_EditCJXX.this, "删除成功！", Toast.LENGTH_SHORT).show();
		} else
		{
			Toast.makeText(Offline_EditCJXX.this, "删除失败！", Toast.LENGTH_SHORT).show();
		}
	...
	private void addPicture(String type, List<String> list, final List<FJ_SCFJ> list_pic, final LinearLayout ll_pic)
	{
		for (int i = 0; i < list.size(); i++)
		{
			...
			imageView.setLayoutParams(lp);
			imageView.setScaleType(ImageView.ScaleType.CENTER_CROP);
			BitmapHelper.setImageView(Offline_EditCJXX.this, imageView, FJBDLJ);
			...
			fj_SCFJ.setGLID(bhq_XHSJCJ.getCJID());
	...
	private void updateData()
	{
		...
		bhq_XHSJCJ.setCJID(bhq_XHSJCJ.getCJID());
		...
		boolean issuccess = SqliteDb.save(Offline_EditCJXX.this, bhq_XHSJCJ);
		if (issuccess)
		{
			...
			{
				boolean success = SqliteDb.saveAll(Offline_EditCJXX.this, list_allpicture_notload);
				if (success)
				{
					Toast.makeText(Offline_EditCJXX.this, "保存成功！", Toast.LENGTH_SHORT).show();
					finish();
				} else
				{
					pb_upload.setVisibility(View.GONE);
					btn_upload.setVisibility(View.VISIBLE);
					Toast.makeText(Offline_EditCJXX.this, "数据保存失败", Toast.LENGTH_SHORT).show();
				}
			} else
			{
				Toast.makeText(Offline_EditCJXX.this, "保存成功！", Toast.LENGTH_SHORT).show();
	...
	{
		list_dic = SqliteDb.getDics(Offline_EditCJXX.this, Dictionary.class);
		if (list_dic == null)
		{
