...
public class Offline_TaskContent extends Activity
{
    ...
    private void setRenWuComplete_ZRR()
    {
        RW_RW rw_rw = new RW_RW();
        rw_rw.setRWID(RWID);
        rw_rw.setRWSFJS("true");
        rw_rw.setRWJSSJ(utils.getTime());
        rw_rw.setXGSJ(utils.getTime());
        rw_rw.setIsUpload("0");
        boolean issuccess = SqliteDb.setRenWuComplete_ZRR(Offline_TaskContent.this, rw_rw, RWID, dt_manager_offline.getid());
        if (issuccess)
        {
