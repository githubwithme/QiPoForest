...
public class Offline_ChangePwd extends Activity
{
	...
	void btn_sure()
	{
		if (!et_oldpwd.getText().toString().equals("") && !et_newpwd.getText().toString().equals("") && !et_confirmnewpwd.getText().equals(""))
		{
			...
			{
				Toast.makeText(Offline_ChangePwd.this, "两次密码不一致!请重新输入", Toast.LENGTH_SHORT).show();
			}
		} else
		{
			Toast.makeText(Offline_ChangePwd.this, "请输入密码!", Toast.LENGTH_SHORT).show();
		}
	...
	public void changePassword()
	{
		...
		} else
		{
			...
			boolean issuccess = SqliteDb.changePassword(Offline_ChangePwd.this, dt_manager_offlinetemp);
			if (issuccess)
			{
				Toast.makeText(Offline_ChangePwd.this, "修改成功!", Toast.LENGTH_SHORT).show();
				CleanLoginInfo(dt_manager_offlinetemp);
				Intent intent = new Intent(Offline_ChangePwd.this, Login_.class);
				startActivity(intent);
				finish();
			} else
			{
				Toast.makeText(Offline_ChangePwd.this, "修改密码失败!", Toast.LENGTH_SHORT).show();
			}
	...
	private void CleanLoginInfo(dt_manager_offline dt_manager_offline)
	{
		...
		SqliteDb.existSystemoffline(Offline_ChangePwd.this, dt_manager_offline);
	}
}
