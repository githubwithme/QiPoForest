...
import com.bhq.R;
import com.bhq.adapter.ListViewEventAdapter;
...
public class EventListFragment extends Fragment
{
	...
	private ListViewEventAdapter listAdapter;
	...
	private void getListData(final int actiontype, final int objtype, final PullToRefreshListView lv, final BaseAdapter adapter, final TextView more, final ProgressBar progressBar, final int PAGESIZE, int PAGEINDEX)
	{
		...
		String params = ConnectionHelper.setParams("APP.GetBHQ_XHSJList", "0", hashMap);
		new HttpUtils().send(HttpRequest.HttpMethod.POST, AppConfig.dataBaseUrl, ConnectionHelper.getParas(params), new RequestCallBack<String>()
		{
			...
			public void onSuccess(ResponseInfo<String> responseInfo)
			{
				...
				List<BHQ_XHSJ> listNewData = null;
				Result result = JSON.parseObject(responseInfo.result, Result.class);
				if (result.getResultCode() == 200)// 连接数据库成功
				{
					listNewData = JSON.parseArray(ResultDeal.getAllRow(result), BHQ_XHSJ.class);
					if (listNewData == null)
					{
	...
	{
		listAdapter = new ListViewEventAdapter(getActivity(), listData, R.layout.eventlist_item);
		...
		frame_listview_plant.setOnItemClickListener(new AdapterView.OnItemClickListener()
		{
			public void onItemClick(AdapterView<?> parent, View view, int position, long id)
			{
				...
				if (bhq_XHSJ == null)
					return;
				Intent intent = new Intent(getActivity(), ShowEventContent_.class);
