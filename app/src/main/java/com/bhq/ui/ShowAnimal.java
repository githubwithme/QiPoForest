 ...
 */
@SuppressLint("NewApi")
@EActivity(R.layout.showanimal)
public class ShowAnimal extends Activity
{
	...
	class FoundationFragment extends Fragment
	{
		...
		public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState)
		{
			...
			tv_description.setText(animal.getDescription());
			tv_bz.setText(animal.getDescription());
			return rootView;
		}
	...
	private void setImage(String glid)
	{
		...
		new HttpUtils().send(HttpRequest.HttpMethod.POST, AppConfig.dataBaseUrl, ConnectionHelper.getParas(params), new RequestCallBack<String>()
		{
			...
			public void onSuccess(ResponseInfo<String> responseInfo)
			{
				...
				{
					AppContext.makeToast(ShowAnimal.this, "error_connectDataBase");
					return;
				}
...
//				ShowAnimal.this.getFragmentManager().beginTransaction().replace(R.id.img_container, pictureScrollFragment).commit();
			}

			@Override
			public void onFailure(HttpException error, String msg)
			{
				Toast.makeText(ShowAnimal.this, "获取失败", Toast.LENGTH_SHORT).show();
			}
		});
	}
	...
	private void getVideo(String glid)
	{
		...
		new HttpUtils().send(HttpRequest.HttpMethod.POST, AppConfig.dataBaseUrl, ConnectionHelper.getParas(params), new RequestCallBack<String>()
		{
			...
			public void onSuccess(ResponseInfo<String> responseInfo)
			{
				...
				for (int i = 0; i < list_fj.size(); i++)
				{
					ProgressBar progressBar = new ProgressBar(ShowAnimal.this, null, android.R.attr.progressBarStyleHorizontal);
	...
	public void downloadVideo(final String ID, String path, final String target, final ProgressBar progressBar)
	{
		...
		http.download(path, target, true, true, new RequestCallBack<File>()
		{
			...
			public void onFailure(HttpException error, String msg)
			{
				...
				{
					Toast.makeText(ShowAnimal.this, "下载失败！找不到文件!", Toast.LENGTH_SHORT).show();
				}
			}
			...
			public void onSuccess(ResponseInfo<File> responseInfo)
			{
				...
				ll_video.removeView(progressBar);
				Toast.makeText(ShowAnimal.this, "下载成功！", Toast.LENGTH_SHORT).show();
				ImageView imageView = new ImageView(ShowAnimal.this);
				LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(400, LayoutParams.MATCH_PARENT, 0); // ,
																													// 1是可选写的
				lp.setMargins(25, 4, 0, 4);
