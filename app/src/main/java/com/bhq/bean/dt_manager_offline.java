 ...
 * @version 1.0
 */
@Table(name = "dt_manager_offline")
public class dt_manager_offline implements Parcelable
{
	@Id
	@NoAutoIncrement
	public String id;
	...
	public static final Parcelable.Creator<dt_manager_offline> CREATOR = new Creator()
	{
		@Override
		public dt_manager_offline createFromParcel(Parcel source)
		{
			// 必须按成员变量声明的顺序读取数据，不然会出现获取数据出错
			dt_manager_offline p = new dt_manager_offline();
			...
			p.setIsLogin(source.readString());
			p.setIsUpload(source.readString());
			return p;
		}

		@Override
		public dt_manager_offline[] newArray(int size)
		{
			return new dt_manager_offline[size];
		}
	};
